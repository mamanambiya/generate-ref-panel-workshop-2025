...
## Federated Genotype Imputation Pipeline - Azure Compatible
...
task ExtractRegion {
    ...
    runtime {
        docker: "biocontainers/bcftools:v1.15.1_cv1"
        memory: "4 GB"
        cpu: 2
...
task MinimacConversion {
    ...
    command <<<
        ...
        # Install Minimac4 in the container
        echo "Installing Minimac4..."
        cd /tmp
        wget -q https://github.com/statgen/Minimac4/releases/download/v1.0.2/minimac4-1.0.2-Linux.sh
        chmod +x minimac4-1.0.2-Linux.sh
        ./minimac4-1.0.2-Linux.sh --skip-license --prefix=/usr/local || true
        
        # Alternative: build from source if binary fails
        if ! command -v minimac4 &> /dev/null; then
            echo "Building Minimac4 from source..."
            apt-get update && apt-get install -y git cmake build-essential zlib1g-dev
...
workflow FederatedImputationPipeline {
    ...
    meta {
        description: "Federated Genotype Imputation Pipeline - Azure Compatible"
        author: "GA4GH Hackathon 2025 - African Genomics Team"
